...
def run(model, train_loader, test_loader,lat_vecs, epochs, optimizer, scheduler, writer,
        device, results_dir, data_mean, data_std, template_face, 
        arap_weight=0.0, use_arap_epoch=800, arap_eig_k=60, continue_train=False,):
    
    start_epoch = 0
    if continue_train:
        start_epoch = writer.load_checkpoint( model, lat_vecs, optimizer, scheduler)

    for epoch in range(start_epoch+1, epochs):
        t = time.time()
        use_arap = epoch > use_arap_epoch

        train_loss, l1_loss, arap_loss, l2_error = train(model,epoch, optimizer, train_loader,lat_vecs, 
                device, results_dir,data_mean, data_std, template_face, 
                arap_weight=arap_weight, arap_eig_k=arap_eig_k, use_arap=use_arap,)
...
def train(model, epoch, optimizer, loader,lat_vecs, device, results_dir,data_mean, data_std, template_face, 
    ...
    for data in loader:
        ...
        tmp_error = torch.sqrt(torch.sum((pred_shape - gt_shape)**2,dim=2)).detach()
        ...
        if use_arap and arap_weight>0:
            ...
            #jacob = get_jacobian_rand2(
            jacob = get_jacobian(
                        out,  batch_vecs, data_mean_gpu, data_std_gpu,
                        model, device,
                        epsilon=1e-3)
...
def get_jacobian_rand2(cur_shape, z, data_mean_gpu, data_std_gpu, model, device, epsilon=[1e-3]):
    nb, nz = z.size()
    _, n_vert, nc = cur_shape.size()
    if nz > 72:
      rand_idx = np.random.permutation(nz)[:72]
      nz = 72
    else:
...
def get_jacobian_rand(out, z,data_mean_gpu, data_std_gpu, model, device, epsilon=1e-3):
    # z = z.view(out.size(0), -1)
    nb, nz = z.size()
    _, n_vert, nc = out.size()
    jacobian = torch.zeros((nb, n_vert*nc, nz)).to(device)
    rand_ids = np.random.randint(0, nz, size=100)
...
def test_reconstruct(
    ...
    device, results_dir, data_mean, data_std, template_face ):
    # load model
    start_epoch = writer.load_checkpoint(model)
